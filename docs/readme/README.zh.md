...
这个生成的文件只包含 OMX 运行时 overlay（`<!-- OMX:RUNTIME:START -->` 到 `<!-- OMX:RUNTIME:END -->`）。`CODEX_HOME/AGENTS.md` 和项目 `AGENTS.md` 不会被复制进去，Codex 通过原生 AGENTS 发现机制只加载一次。Team worker 仍自行组合其 worker 指示。
